...
x_arc = [transpose(x), zeros(length(x), 1), zeros(length(x),1), transpose(traj_time)];
y_arc = [transpose(y), zeros(length(y), 1), zeros(length(y),1), transpose(traj_time)];
...
[   q1_trajectory, q1_velocity, q1_acceleration, ...
    ...
    GenerateArbitraryTrajectory(x_arc, y_arc, delta_t, l1, l2);
